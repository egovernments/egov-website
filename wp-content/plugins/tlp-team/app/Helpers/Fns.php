...
class Fns {
	 ...
	 * @return string|void
	 */
	public static function render_view( $view_name, $args = array(), $return = false ) {
		$path           = str_replace( '.', '/', $view_name );
		$resources_path = rttlp_team()->plugin_path() . '/resources/' . $path . '.php';

		if ( ! file_exists( $resources_path ) ) {
			return new \WP_Error( 'brock', __( "$resources_path file not found", 'tlp-team' ) );
		}
	 ...
	 * @return void|string
	 */
	public static function rtFieldGenerator( $fields = array() ) {
		$html = null;
	 ...
	 */
	public static function sanitize( $field = array(), $value = null ) {
		$newValue = null;
		if ( is_array( $field ) ) {
				...
				if ( $type == 'text' || $type == 'number' || $type == 'select' || $type == 'checkbox' || $type == 'radio' ) {
					...
					foreach ( $value as $k => $v ) {
						...
						if ( $k == 'color' ) {
							...
							$nV = self::sanitize( array( 'type' => 'text' ), $v );
						}
						if ( $nV ) {
							$newValue[ $k ] = $nV;
						}
	...
	public static function custom_pagination( $pages = '', $range = 4 ) {
		$html      = null;
		$showitems = ( $range * 2 ) + 1;
		global $paged;
		if ( is_front_page() ) {
	...
	public static function memberDetailGallery( $post_id = null ) {
		...
		$fields   = isset( $settings['detail_page_fields'] ) ? $settings['detail_page_fields'] : array();
	...
	public static function getAllTermsByTaxonomyName( $taxonomy ) {
		...
		$terms = array();
		if ( $taxonomy ) {
			$termList = get_terms( array( rttlp_team()->taxonomies[ $taxonomy ] ), array( 'hide_empty' => 0 ) );
	...
	public static function getFeatureImageSrc( $post_id, $fImgSize = 'medium', $defaultImgId = null, $customImgSize = array() ) {
	 ...
	 * @return string|null
	 */
	public static function getFeatureImageHtml( $post_id, $fImgSize = 'medium', $defaultImgId = null, $customImgSize = array(), $lazy = false ) {
		...
		if ( ! $imgHtml ) {
			...
			$attr          = isset( $attr['src'] ) ? apply_filters( 'wp_get_attachment_image_attributes', $attr, false, $fImgSize ) : array();
	...
	public static function get_formatted_contact_info( $items = array(), $fields ) {
		$contact_info = null;
		if ( ! empty( $items['email'] ) && in_array( 'email', $fields ) ) {
			$contact_info .= '<li><i class="far fa-envelope"></i><a href="mailto:' . $items['email'] . '"><span class="tlp-email">' . $items['email'] . '</span></a></li>';
		}
		if ( ! empty( $items['telephone'] ) && in_array( 'telephone', $fields ) ) {
			$contact_info .= '<li><i class="fa fa-phone"></i><a href="tel:' . $items['telephone'] . '" class="tlp-phone">' . $items['telephone'] . '</a></li>';
		}
		if ( $items['mobile'] && in_array( 'mobile', $fields ) ) {
			$contact_info .= "<li class='tlp-mobile'><i class='fa fa-mobile'></i> <a href='tel:" . $items['mobile'] . "'>" . $items['mobile'] . '</a></li>';
		}
		if ( $items['fax'] && in_array( 'fax', $fields ) ) {
			$contact_info .= "<li class='tlp-fax'><i class='fa fa-fax'></i> <a href='fax:" . $items['fax'] . "'><span>{$items['fax']}</span></a></li>";
		}
		if ( ! empty( $items['location'] ) && in_array( 'location', $fields ) ) {
			$contact_info .= '<li><i class="fa fa-map-marker"></i><span class="tlp-location">' . $items['location'] . '</span></li>';
		}
		if ( ! empty( $items['web_url'] ) && in_array( 'web_url', $fields ) ) {
			$contact_info .= '<li><a target="_blank" href="' . $items['web_url'] . '"><i class="fa fa-globe"></i><span class="tlp-url">' . $items['web_url'] . '</span></a></li>';
		}
	...
	public static function get_formatted_social_link( $sLink, $fields ) {
		...
		if ( ! empty( $sLink ) && is_array( $sLink ) && in_array( 'social', $fields ) ) {
			...
			foreach ( $sLink as $id => $itemLink ) {
				...
				if ( $lID == 'envelope-o' ) {
					$lURL = ! empty( $itemLink['url'] ) ? esc_attr( $itemLink['url'] ) : null;
					$lURL = 'mailto:' . $lURL;
				}
				...
				if ( $lID != 'google-plus' && $icon_class ) {
					$html .= "<a href='{$lURL}' title='{$lID}' target='_blank'><i class='{$icon_class}'></i></a>";
				}
			}
			$html .= '</div>';
		}
