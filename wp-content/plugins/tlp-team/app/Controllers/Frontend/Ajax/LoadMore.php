...
class LoadMore {
	...
	protected function init() {
		add_action( 'wp_ajax_ttp_Layout_Ajax_Action', array( $this, 'response' ) );
		add_action( 'wp_ajax_nopriv_ttp_Layout_Ajax_Action', array( $this, 'response' ) );
	}
	...
	public function response() {
		...
		if ( Fns::verifyNonce() ) {
			...
			if ( $scID && ! is_null( get_post( $scID ) ) ) {
				...
				$allCol     = ! empty( $scMeta['ttp_column'][0] ) ? unserialize( $scMeta['ttp_column'][0] ) : array();
				...
				/* Argument create */
				$args                = array();
				$args['post_type']   = array( rttlp_team()->post_type );
				...
				$taxQ        = array();
				$departments = ( isset( $scMeta['ttp_departments'] ) ? $scMeta['ttp_departments'] : array() );
				...
				$designation = ( isset( $scMeta['ttp_designations'] ) ? $scMeta['ttp_designations'] : array() );
				...
				if ( $isCarousel ) {
					$cOpt                   = isset( $scMeta['carousel'][0] ) ? unserialize( $scMeta['carousel'][0] ) : array();
				...
				$action_term     = ! empty( $_REQUEST['term'] ) ? ( $_REQUEST['term'] == 'all' ? 'all' : absint( $_REQUEST['term'] ) ) : 0;
				...
				if ( $sAction ) {
					...
					add_filter( 'posts_where', array( $this, 'tlp_team_search_where' ) );
					add_filter( 'posts_join', array( $this, 'tlp_team_search_join' ) );
					add_filter( 'posts_groupby', array( $this, 'tlp_team_search_groupby' ) );
				...
				if ( $sAction ) {
					remove_filter( 'posts_where', array( $this, 'tlp_team_search_where' ) );
					remove_filter( 'posts_join', array( $this, 'tlp_team_search_join' ) );
					remove_filter( 'posts_groupby', array( $this, 'tlp_team_search_groupby' ) );
				}
