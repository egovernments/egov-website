...
class Shortcode {
	...
	function team_shortcode( $atts ) {
		...
		if ( $scID && ! is_null( get_post( $scID ) ) ) {
			...
			$args = QueryArgs::buildArgs( $buildMetas, $isCarousel );
			...
			if ( $teamQuery->have_posts() ) {
				...
				if ( $isCarousel ) {
					...
					if ( 'carousel9' === $layout || 'carousel11' === $layout ) {
						$navPosition = 'bottom';
					}
				...
				while ( $teamQuery->have_posts() ) :
					...
					$arg['tlp_skill']   = $skill ? maybe_unserialize( $skill ) : array();
				...
				if ( $pagination && ! $isCarousel ) {
					...
					if ( $htmlUtility ) {
						$l4toggle = null;
						if ( $layout == 'layout4' ) {
							$l4toggle = "data-l4toggle='{$l4toggle}'";
						}
						$html .= "<div class='rt-pagination-wrap' data-total-pages='{$totalPage}' data-posts-per-page='{$postPp}' data-type='{$posts_loading_type}' {$l4toggle} >" . $htmlUtility . '</div>';
					}
				}
	...
	private function metas( array $meta ) {
		return array(
			...
			'allCol'             => ! empty( $meta['ttp_column'][0] ) ? unserialize( $meta['ttp_column'][0] ) : array(),
			'popupBg'            => ! empty( $meta['ttp_popup_bg_color'][0] ) ? esc_attr( $meta['ttp_popup_bg_color'][0] ) : '',
			'postIn'             => ! empty( $meta['ttp_post__in'] ) && is_array( $meta['ttp_post__in'] ) ? $meta['ttp_post__in'] : array(),
			'postNotIn'          => ! empty( $meta['ttp_post__not_in'] ) && is_array( $meta['ttp_post__not_in'] ) ? $meta['ttp_post__not_in'] : array(),
			...
			'department_ids'     => isset( $meta['ttp_departments'] ) ? $meta['ttp_departments'] : array(),
			'designation_ids'    => isset( $meta['ttp_designations'] ) ? $meta['ttp_designations'] : array(),
			...
			'customImgSize'      => ! empty( $meta['ttp_custom_image_size'][0] ) ? unserialize( $meta['ttp_custom_image_size'][0] ) : array(),
			'visibility'         => ! empty( $meta['ttp_selected_field'] ) ? $meta['ttp_selected_field'] : array( 'name', 'designation', 'short_bio', 'social' ),
			'filters'            => ! empty( $meta['ttp_filter'] ) ? $meta['ttp_filter'] : array(),
			...
			'cOpt'               => ! empty( $meta['ttp_carousel_options'] ) ? $meta['ttp_carousel_options'] : array(),
